...
{
    int devDesc;
    devDesc = libusb_control_transfer(handle, 0x80, GetStatus, 0, 0, buffer, 4, 0);
...
{
    int devDesc = libusb_control_transfer(handle, 0x80, GetStatus, 0, intf.bInterfaceNumber, buffer, 4, 0);
...
{
    int devDesc = libusb_control_transfer(handle, 0X82, GetStatus, 0, ep.bEndpointAddress, buffer, 4, 0);
...
void usb_read_string(libusb_device_handle *handle, struct libusb_endpoint_descriptor epin)
{
    ...
    while (20 != count)
    {
        if (ep_out_status(handle, epin) >= 0)
        {
            clear_halt(handle, epin);
        }
        int transfer = libusb_bulk_transfer(handle, epin.bEndpointAddress, rd_buf, epin.wMaxPacketSize, datalen, 0);
...
void usb_send_string(libusb_device_handle *handle, struct libusb_endpoint_descriptor epin, struct libusb_device_descriptor devDesc, char str[])
{
    int tx;
    printf("****** Wait Sending**********\n\n");
    ...
    {
        clear_halt(handle, epin);
        if (libusb_bulk_transfer(handle, epin.bEndpointAddress, getchar_2str(str[i]), 1, 0, 0) == 0)
        {
            printf("%x ", i);
        }
        usleep(100000);
